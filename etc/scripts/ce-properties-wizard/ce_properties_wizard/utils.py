...
def find_ce_lib_directory() -> Path:
    ...
    def validate_lib_dir(path: Path) -> bool:
        ...
        return (compilers_dir.exists() and compilers_dir.is_dir() and 
                any(compilers_dir.glob("*.ts")))
...
class VersionExtractor:
    ...
    PATTERNS = {
        ...
        'intel': [
            r"(?:icc|icpc|icx|dpcpp).*?(\d+\.\d+\.\d+)",
            r"intel.*?compiler.*?(\d+\.\d+)"
        ],
        'intel_fortran': [
            r"(?:ifx|ifort)\s*\([^)]+\)\s*(\d+\.\d+\.\d+)",
            r"(?:ifx|ifort).*?(\d+\.\d+\.\d+)"
        ],
        'msvc': [
            r"compiler version (\d+\.\d+\.\d+)"
        ],
        'nvcc': [
            r"release (\d+\.\d+)"
        ],
        ...
        'go': [
            r"go\s*version\s+go(\d+\.\d+(?:\.\d+)?)",
            r"go(\d+\.\d+(?:\.\d+)?)"
        ],
        'tinygo': [
            r"(?:tinygo\s+)?version:?\s+(\d+\.\d+(?:\.\d+)?)"
        ],
        'python': [
            r"python (\d+\.\d+\.\d+)",
            r"pypy.*?(\d+\.\d+\.\d+)"
        ...
        'fpc': [
            r"Free Pascal Compiler version (\d+\.\d+\.\d+)",
            r"fpc.*?(\d+\.\d+\.\d+)"
        ],
        'z88dk': [
            r"z88dk.*?-\s*v([^-\s]+(?:-[^-\s]+)*)",
