...
get_gdc() {
    ...
    mkdir "${OPT}"/gdc
    pushd "${OPT}"/gdc
    fetch "https://gdcproject.org/downloads/binaries/${VER}/x86_64-linux-gnu/gdc-${VER}+${BUILD}.tar.xz" | tar Jxf -
    popd
}
...
if ! ("${OPT}"/rust/bin/rustc -V | grep -q -F '1.35.0'); then
    install_new_rust 1.35.0
fi
